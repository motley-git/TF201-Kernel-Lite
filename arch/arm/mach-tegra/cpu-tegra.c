...
#define SYSTEM_OVERCLOCK_1P7G_MODE	(5)
#define SYSTEM_OVERCLOCK_1P5G_MODE_MAX_FREQ		(1500000)
#define SYSTEM_OVERCLOCK_1P6G_MODE_MAX_FREQ		(1600000)
#define SYSTEM_OVERCLOCK_1P7G_MODE_MAX_FREQ		(1700000)
#define SYSTEM_NORMAL_MODE_MAX_FREQ		(1400000)
...
static int system_mode_set(const char *arg, const struct kernel_param *kp)
{
	...
	if (ret == 0) {
		...
		if( (system_mode<SYSTEM_NORMAL_MODE) || (system_mode>SYSTEM_OVERCLOCK_1P7G_MODE))
 ...
 unsigned int ASUS_governor_speed(unsigned int requested_speed)
{
	...
	else  if( (system_mode==SYSTEM_OVERCLOCK_1P7G_MODE ) && ( requested_speed > SYSTEM_OVERCLOCK_1P7G_MODE_MAX_FREQ ))
		new_speed=SYSTEM_OVERCLOCK_1P7G_MODE_MAX_FREQ;
#endif
...
#ifdef ASUS_OVERCLOCK
	if(system_mode == SYSTEM_OVERCLOCK_1P5G_MODE || system_mode == SYSTEM_OVERCLOCK_1P6G_MODE || system_mode == SYSTEM_OVERCLOCK_1P7G_MODE)
	{
		if(edp_enable)
		{
...
int tegra_cpu_late_resume_set_speed_cap(int speed)
{
	...
	if(system_mode == SYSTEM_OVERCLOCK_1P5G_MODE ||
			...
			system_mode == SYSTEM_OVERCLOCK_1P7G_MODE){
		if(edp_enable){
			pr_info("%s : EDP enable\n", __func__);
			new_speed = edp_governor_speed(new_speed);
		}
